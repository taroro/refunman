...
export default class WaitingList extends Component {
  ...
  componentDidMount() {
    this.unsubscribeWaitingList = this.refQuotations.onSnapshot(this._onQuotationsCollectionUpdate)
  }

  componentWillUnmount() {
    this.unsubscribeWaitingList()
  }
          ...
          <View style={[styles.postList, {flexDirection: "column", elevation:(Platform.OS === 'ios'?0:2)}]}>
            <View style={{flex: 3, flexDirection: 'row', marginTop: 5, marginBottom: 5, justifyContent: 'center', alignItems: 'center'}}>
              <View style={{flex: 1, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start'}}> 
                ...
                <View style={{flex: 2, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 5}}>
                  <View><Icon name='watch-later' size={40} backgroundColor={theme.COLOR_WHITE} color={theme.COLOR_GREY} /></View>
                  <Text style={[styles.textTiny, {color: theme.COLOR_GREY, textAlign: 'center', width: '100%'}]}>รอการ{'\n'}ตอบรับ</Text>
                </View>
                :
                <View style={{flex: 2, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 5}}>
                  <View><Icon name='check-circle' size={40} backgroundColor={theme.COLOR_WHITE} color={theme.PRIMARY_COLOR} /></View>
                  <Text style={[styles.textTiny, {color: theme.PRIMARY_COLOR, textAlign: 'center', width: '100%'}]}>รอการยืนยัน</Text>
                </View>
                }
